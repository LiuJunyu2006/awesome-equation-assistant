...
inline bool is_true(const Decimal v);
inline bool is_false(const Decimal v);

template <typename Iterator>
inline bool string_to_real(Iterator& itr_external, const Iterator end, Decimal& t,
...
namespace io {
namespace details {
inline void print_type(const std::string& fmt, const Decimal& v, exprtk::details::numeric::details::my_real_type_tag);
}
}  // namespace io
}  // namespace rtl
...
template <>
struct number_type<Decimal> {
    typedef my_real_type_tag type;
};

template <>
struct epsilon_type<my_real_type_tag> {
    static inline Decimal value() {
        const Decimal epsilon = Decimal(0.000000000001);
        return epsilon;
    }
};

inline bool is_nan_impl(const Decimal& v, my_real_type_tag) {
    return v != v;
}
...
inline T acos_impl(const T v, my_real_type_tag) {
    return boost::multiprecision::acos(v);
}
template <typename T>
inline T acosh_impl(const T v, my_real_type_tag) {
    return boost::multiprecision::log(v + boost::multiprecision::sqrt((v * v) - Decimal(1)));
}
template <typename T>
inline T asin_impl(const T v, my_real_type_tag) {
    return boost::multiprecision::asin(v);
}
template <typename T>
...
inline T atan_impl(const T v, my_real_type_tag) {
    return boost::multiprecision::atan(v);
}
template <typename T>
inline T atanh_impl(const T v, my_real_type_tag) {
    return (boost::multiprecision::log(Decimal(1) + v) - log(Decimal(1) - v)) / Decimal(2);
}
template <typename T>
inline T ceil_impl(const T v, my_real_type_tag) {
    return boost::multiprecision::ceil(v);
}
template <typename T>
inline T cos_impl(const T v, my_real_type_tag) {
    return boost::multiprecision::cos(v);
}
template <typename T>
inline T cosh_impl(const T v, my_real_type_tag) {
    return boost::multiprecision::cosh(v);
}
template <typename T>
inline T exp_impl(const T v, my_real_type_tag) {
    return boost::multiprecision::exp(v);
}
template <typename T>
inline T floor_impl(const T v, my_real_type_tag) {
    return boost::multiprecision::floor(v);
}
template <typename T>
...
inline T log2_impl(const T v, my_real_type_tag) {
    return boost::multiprecision::log(v) / Decimal(boost::multiprecision::log(Decimal(2)));
}
template <typename T>
...
inline T sin_impl(const T v, my_real_type_tag) {
    return boost::multiprecision::sin(v);
}
template <typename T>
inline T sinh_impl(const T v, my_real_type_tag) {
    return boost::multiprecision::sinh(v);
}
template <typename T>
...
inline T trunc_impl(const T v, my_real_type_tag) {
    return Decimal(static_cast<long long>(v));
}

template <typename T>
inline T const_pi_impl(my_real_type_tag) {
    return boost::multiprecision::acos(Decimal(-1));
}
template <typename T>
inline T const_e_impl(my_real_type_tag) {
    return Decimal(
...
inline bool is_true_impl(const Decimal v) {
    return (0.0 != v);
}

inline bool is_false_impl(const Decimal v) {
    return (0.0 == v);
}
...
inline T ncdf_impl(T v, my_real_type_tag) {
    T cnd = T(0.5) * (T(1) + erf_impl(abs_impl(v, my_real_type_tag()) / T(boost::multiprecision::sqrt(2)), my_real_type_tag()));
...
inline T pow_impl(const T v0, const T v1, my_real_type_tag) {
    return boost::multiprecision::pow(v0, v1);
}
...
inline bool string_to_real(Iterator& itr_external, const Iterator end, Decimal& t,
    ...
    return string_to_real<Iterator, double>(itr_external, end, t, num_type);
}
